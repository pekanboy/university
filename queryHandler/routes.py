...
@queryHandler.route('/all-clients-not-connect-for-period', methods=["GET", "POST"])
@group_permission_validation_decorator
def allClientsNotConnectForPeriod():
    if request.method == "GET":
        return render_template('queryHandler/dateUser.html', description=config[0].get('name'))
...
@queryHandler.route('/all-services-where-price-bigger', methods=["GET", "POST"])
@group_permission_validation_decorator
def allServicesWherePriceBigger():
    if request.method == "GET":
        return render_template('queryHandler/servicePriceBigger.html', description=config[1].get('name'))
